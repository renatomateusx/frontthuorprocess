  ...
  methods: {
    ...
    async enviarMensagemAcao(id, id_mensagem) {
      if (id_mensagem > 0) {
        ...
        const DadosBoleto = JSON.parse(
          this.gridData.find(x => x.id == id).json_gw_response
        );
        const LBarCode = this.getBarCodeBoleto(DadosBoleto);
        const LProdutos = JSON.parse(
          this.gridData.find(x => x.id == id).json_front_end_user_data
        ).produtos;
        const Link = JSON.parse(this.gridData.find(x => x.id == id).json_gw_response);
        const LlinkBoleto = await this.getLinkBoleto(Link);
        var LprodutoTitle = await this.getProdutoName(LProdutos);
        const Nome = JSON.parse(this.gridData.find(x => x.id == id).json_front_end_user_data).dadosComprador.nome_completo.split(" ")[0];
        Mensagem = Mensagem.replace("{first_name}", "%0a" + Nome + "%0a");
        ...
        const LURLFinal = LURL.replace("{@PHONE}", Telefone).replace("{@MENSAGEM}", Mensagem);
        var win = window.open(LURLFinal, "_blank");
        win.focus();
      }
    },
