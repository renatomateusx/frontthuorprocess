        ...
        <form @submit.prevent="validateBeforeSubmit('campanha')" data-vv-scope="campanha">
          ...
          <div v-for="(id_sequencia, i) in sequenciasArray" :key="i">
            <div class="mb-1">
              ...
              >Sequência {{sequenciasArray[i].id_sequencia}}</span>
            </div>
            <sequencia-card
              @AddSequencia="AdicionarSequencia($event)"
              :id="sequenciasArray[i].id_sequencia"
          ...
          API_CAMPANHA_BOLETO.GetCampanhasBoletoRecover()
            .then(resCampanhaCarrinho => {
              if (resCampanhaCarrinho.data) {
                ...
                resCampanhaCarrinho.data.sequencia.campanhaSequenciaArray.forEach(
                  (obj, i) => {
            ...
            .catch(error => {
              console.log(
                "Erro ao pegar as informações da campanha carrinho abandonado",
                error
              );
            });
        })
