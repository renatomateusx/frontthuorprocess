...
<template>
  <ContentWrapper>
    ...
    <small>
      Todos os Down Selling criados por você estão aqui.
      <br />Para criar um novo DownSell, basta clicar no botão de 'Novo DownSell'.
    </small>
    <p></p>
    <div class="float-left pull-left mb-3">
      <button class="btn btn-danger btn-lg" v-on:click="excluirDownSellSelecionado()" v-show="selectedDownSell > 0">
        <span class="fa fa-trash"></span> Excluir Selecionado
      </button>
    </div>
     <div class="float-left pull-left mb-3">
      <button class="btn btn-danger btn-lg" v-on:click="instalarDownSellApp()">
        <span class="fas fa-cart-arrow-down"></span> Instalar App
      </button>
    </div>
    <div class="float-right pull-right mb-3">
      <button class="btn btn-primary btn-lg" v-on:click="adicionarNovoDownSell()">
        <span class="fa fa-plus"></span> Novo DownSell
      </button>
    </div>
      ...
      <div id="grid-template">
        ...
        <div class="table-body-wrapper">
          <table class="table-body">
            <tbody>
              <tr
                v-for="{id, status, tipo_checkout, nome, produto_from, produto_to} in dataPerPage" class="cursorP" v-on:click="SelectDownSell(id, nome)"
              >
                <td style="width: 200px!important;" class="data padding1010">
                   <router-link :to="{path: '/marketing/downsell/edit/' + getCripto(id, status)}">
...
export default {
  ...
  methods: {
    ...
    adicionarNovoDownSell() {
      this.$router.push("/marketing/downsell/add");
    },
    SelectDownSell(id, nome) {
      this.selectedDownSell = id;
      this.nomeselectedDownSell = nome;
    },
    excluirDownSellSelecionado(){
      API_NOTIFICATION.showConfirmDialog('EXCLUSÃO', 'Deseja realmente excluir ' + this.nomeselectedDownSell + '?', 'warning', this.ExcluirDownSell);
    },
    ExcluirDownSell(){
      API_MKT.DeleteDownSellByID(this.selectedDownSell)
      .then((resExclude)=>{
      ...
      .catch((error)=>{
        console.log("Erro ao excluir o UpSell " + this.nomeselectedDownSell, error);
      })
    },
    instalarDownSellApp(){
      API_NOTIFICATION.showNotificationConfirm("Thuor Down Sell", "Você Está Prestes a Instalar o App Down Sell na Sua Loja. Continuar?", "question", ()=>{
        API_INTEGRACAO_SHOPIFY_APPS.InstalaAppThuorDownSellShopify()
        .then((res)=>{
